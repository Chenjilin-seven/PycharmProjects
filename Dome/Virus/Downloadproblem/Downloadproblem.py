...
driver.execute_script("""
    document.querySelector("a[href='javascript:void(0);'][onclick=\\"setCookie('xmenu','4,noiptg')\\"]").click();
""")
...
for item in findlist:
    ...
    html_all = webdriver.Chrome(options=option)
    html_all.get(path)

    html = html_all.find_element(By.CSS_SELECTOR, 'td[class="pcontent"]')
    with open(f"downloaded_problem\\{title}.html","w",encoding="utf-8") as f:
        f.write(html.get_attribute("outerHTML"))
    html_all.quit()
driver.quit()